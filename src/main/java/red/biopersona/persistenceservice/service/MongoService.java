...
public class MongoService {
	...
	public boolean canOperateTheClient(String Client) {		
		...
		Bson filtro = Filters.and(
				Filters.eq("companyName", Client),
				Filters.eq("active", true),
				Filters.or(
						Filters.exists("maxDateUso",false),
						Filters.gte("maxDateUso", new Date())		
						)
		...
		if (!clients_lists.isEmpty()) {
			resultado = true;
			log.info(clients_lists.get(0).getString("companyName"));
		}
		return resultado;
	}
	...
	public Object[] DeleteBiometric(String RmBiometricId, String NameDB) {
		...
		List<Document> ListFiltroPersonIdSample= relationships.find(FiltroPersonIdSample).into(new ArrayList<Document>());
